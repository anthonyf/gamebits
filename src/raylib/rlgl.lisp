(uiop:define-package #:gamebits/rlgl
  (:use #:cl)
  (:export #:rl-matrix-mode
	   #:rl-push-matrix
	   #:rl-pop-matrix
	   #:with-matrix-push
	   #:rl-load-identity
...
(cffi:defcfun ("rlMatrixMode" rl-matrix-mode) :void (mode :int))

;; RLAPI void rlPushMatrix(void);                          // Push the current matrix to stack
(cffi:defcfun ("rlPushMatrix" rl-push-matrix) :void)

;; RLAPI void rlPopMatrix(void);                           // Pop latest inserted matrix from stack
(cffi:defcfun ("rlPopMatrix" rl-pop-matrix) :void)

(defmacro with-matrix-push (&body body)
  `(progn
     (rl-push-matrix)
     (unwind-protect
	  (progn ,@body)
...
(cffi:defcfun ("rlLoadIdentity" rl-load-identity) :void)

;; RLAPI void rlTranslatef(float x, float y, float z);     // Multiply the current matrix by a translation matrix
(cffi:defcfun ("rlTranslatef" rl-translatef) :void (x :float) (y :float) (z :float))

;; RLAPI void rlRotatef(float angle, float x, float y, float z); // Multiply the current matrix by a rotation matrix
(cffi:defcfun ("rlRotatef" rl-rotatef) :void (angle :float) (x :float) (y :float) (z :float))

;; RLAPI void rlScalef(float x, float y, float z);         // Multiply the current matrix by a scaling matrix
(cffi:defcfun ("rlScalef" rl-scalef) :void (x :float) (y :float) (z :float))
